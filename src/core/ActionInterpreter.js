    ...
    async run(source, eventData, collidedTarget = null) {
        ...
        if (!source.scene.scene.isActive()) {
            if (eventData.trigger === 'onReady') {
                // ★ onReady の場合はアクティブになるまで最大 5 フレーム待機する
                let retries = 5;
                while (retries > 0 && !source.scene.scene.isActive()) {
                    // console.log(`[ActionInterpreter.run] Scene not active. Retrying onReady... (${retries} attempts left)`);
                ...
                console.warn(`[ActionInterpreter.run] Guard triggered: Source scene '${source.scene.scene.key}' is not active for trigger '${eventData.trigger}'. Aborting.`);
                return;
            }
        }

...
export default class NovelOverlayScene extends Phaser.Scene {
    ...
    create() {
        ...
        // ★ リセットタグの登録 
        this.scenarioManager.registerTag('restart_trial', async (manager, params) => {
            console.log('[NovelOverlay] restart_trial tag executed.');
            const trialScene = this.scene.get('TrialScene');
            if (trialScene) {
                trialScene.events.emit('RESTART_DEBATE_REQUEST');
            }
        });
